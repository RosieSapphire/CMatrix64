...
void run_benchmark(void)
{
	...
	for (int i = 0; i < 7; i++) {
		int   nchar = sizes[i];
		float t	    = measure(text_noformat, nchar);
		debugf("text_noformat(%d): %d us\n", nchar, (int)(t + 0.5f));
	}
	for (int i = 0; i < 7; i++) {
		int   nchar = sizes[i];
		float t	    = measure(text_format, nchar);
		debugf("text_wordwrap(%d): %d us\n", nchar, (int)(t + 0.5f));
	}
	for (int i = 0; i < 7; i++) {
		int nchar = sizes[i];
		partext	  = rdpq_paragraph_build(
				  &(rdpq_textparms_t) {
						  .line_spacing = -3,
						  .width	= 200,
						  .wrap		= WRAP_WORD,
...
int main(void)
{
	...
	rdpq_font_style(fnt1,
			0,
			&(rdpq_fontstyle_t) {
					.color = RGBA32(0xED, 0xAE, 0x49, 0xFF),
			});
	rdpq_font_style(fnt5,
			0,
			&(rdpq_fontstyle_t) {
	...
	while (1) {
		...
		rdpq_fill_rectangle(par->bbox.x0 + x0,
				    par->bbox.y0 + y0,
				    par->bbox.x1 + x0,
				    par->bbox.y1 + y0);
